...
class BcfFormatStruct:
    def __init__(self, id, n1, n2, data_type, description):
        self.id = id
        self.in_num_1 = n1
        self.out_num_2 = n2
        self.type = data_type
...
def generate_bcf_formats():
    ...
    output.append("#ifndef bcf_formats_step1_INCLUDED")
    output.append("#define bcf_formats_step1_INCLUDED")

    output.append("#include <array>")
    output.append("#include <ostream>")
    output.append("#include <string>")
    output.append("#include <vector>")
    output.append("#include <assert.h>")

    output.append("namespace bcfrec {")
    
    output.append(f"static const unsigned int FILTER_NUM = {len(FILTER_VEC)};")

    output.append("enum FILTER_ENUM {")
    for filter in FILTER_VEC:
        format_key = filter[0]
        output.append(f"    {format_key},")
    output.append("};")
    
    output.append("const char *const FILTER_IDS[] = {")
    for filter in FILTER_VEC:
        filter_key = filter[0]
        output.append(f"    \"{filter_key}\",")
    output.append("};")
    
    output.append("const char *const FILTER_LINES[] = {")
    for filter in FILTER_VEC:
        filter_key = filter[0]
    ...
    output.append("#ifdef OUTPUT_EXTRA_VCF_INFO")
    output.append("const char *const FILTER_DESCS[] = {")
    for filter in FILTER_VEC:
    ...
    output.append(f"static const unsigned int FORMAT_NUM = {len(FORMAT_VEC)};")
    
    output.append("enum FORMAT_ENUM {")
    for fmt in FORMAT_VEC:
        output.append(f"    {fmt['id']},")
    output.append("};")

    output.append("const char *const FORMAT_STRING_PER_REC = \"")
    itnum = 0
    for fmt in FORMAT_VEC:
    ...
    output.append("const char *const FORMAT_STRING_PER_REC_WITHOUT_SSCS = \"")
    itnum = 0
    for fmt in FORMAT_VEC:
    ...
    output.append("const char *const FORMAT_IDS[] = {")
    for fmt in FORMAT_VEC:
        output.append(f"    \"{fmt['id']}\",")
    output.append("};")
    
    output.append("struct BcfFormat {")
    output.append("    bool enable_tier2_consensus_format_tags = False;")
    
    for fmt in FORMAT_VEC:
        type_str = fmt['type']
        id_str = fmt['id']
        if fmt['in_num_1'] == 0 or fmt['in_num_1'] == 1:
            output.append(f"    {type_str} {id_str} = {type_str}();")
        elif fmt['in_num_1'] > 1:
            output.append(f"    std::array<{type_str}, {fmt['in_num_1']}> {id_str} = {{{type_str}()}};")
        else:
            output.append(f"    std::vector<{type_str}> {id_str};")
    
    output.append("};")
    
    output.append("static int streamAppendBcfFormat(std::string &outstring, const BcfFormat &fmt) {")
    itnum = 0
    for fmt in FORMAT_VEC:
        if fmt['is_not_in_out_vcf']:
            ...
            itnum += 1
            continue
        addcheck = f"if ({'fmt.enable_tier2_consensus_format_tags' if fmt['is_SSCS_required'] else 'True'})"
    ...
    output.append("static int resetBcfFormatD(BcfFormat &fmt) {")
    for fmt in FORMAT_VEC:
        if fmt['in_num_1'] == 'D':
            output.append(f"    fmt.{fmt['id']}.clear();")
    output.append("    return 0;")
    output.append("};")

    output.append("static int streamFrontPushBcfFormatR(BcfFormat &dst, const BcfFormat &src) {")
    for fmt in FORMAT_VEC:
        if fmt['in_num_1'] == 'R':
    ...
    output.append("const char *const FORMAT_LINES[] = {")
    for fmt in FORMAT_VEC:
        output.append(f"    \"##FORMAT=<ID={fmt['id']},Number={vcf_number_to_header_str(fmt['out_num_2'])},Type={fmt['type']},Description=\\\"{fmt['description']}\\\">\",")
    output.append("};")
    
    output.append("#ifdef OUTPUT_EXTRA_VCF_FORMAT")
    output.append("const int FORMAT_NUMBERS[] = {")
    for fmt in FORMAT_VEC:
        output.append(f"    {fmt['out_num_2']},")
    output.append("};")
    
    output.append("const char *const FORMAT_DATA_TYPES[] = {")
    for fmt in FORMAT_VEC:
        output.append(f"    \"{fmt['type']}\",")
    output.append("};")
    
    output.append("const char *const FORMAT_DESCRIPTIONS[] = {")
    for fmt in FORMAT_VEC:
